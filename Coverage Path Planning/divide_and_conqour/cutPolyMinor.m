...
vertices = vertices';
center = centroid(vertices')'; % calculate centroid
dp = (vertices-center);
[xxyy] = sum(dp.^2,2);
xy = sum(dp(1,:).*dp(2,:),2);
I = [xxyy(1) xy;xy xxyy(2)];
[V,~] = eig(I); % find principal axis
theta_cut = -atan2(V(1,1),V(2,1)); % along the minor
rho_cut = cos(theta_cut)*center(1)+sin(theta_cut)*center(2); % offset of a cut-line
edge = [1:size(vertices,2) ;circshift(1:size(vertices,2),-1)];
...
for i = 1:2
    ...
    while ~loop_break
        ...
        if (abs(theta_cut-theta_look_ahead)<0.00000001)||(abs(abs(theta_cut-theta_look_ahead)-pi)<0.00000001) % check whether anagles have same slope
    ...
    % search again but this time counter clock-wise
    while ~loop_break
        ...
        A = [cos(theta_look_ahead) sin(theta_look_ahead);
            cos(theta_cut) sin(theta_cut)];
        b = [rho_look_ahead;rho_cut];
        if (abs(theta_cut-theta_look_ahead)<0.00000001)||(abs(abs(theta_cut-theta_look_ahead)-pi)<0.00000001)
            ...
            if cos(cut_direction-theta_look_ahead)<0
                ...
                distance_look_ahead = sum((look_ahead-current).^2);
                distance_cut = sum((cut_intersect'-current).^2);
                if ((distance_cut <= distance_look_ahead) || (abs(distance_look_ahead-distance_cut)<0.00000000001))&&(norm(cut_intersect-current)>0.00000001)
                    ver_list_plus = [ver_list_plus; cut_intersect'];
                    loop_break = true;
                    if (norm(distance_cut-distance_look_ahead)<0.00000001)
                        if idx ~= num_ver
                            next_idx = idx+1;
                        else
                            next_idx = 1;
