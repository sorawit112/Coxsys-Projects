...
if size(region_cm,1)==1
    EdgeTable = table([],[],'VariableNames',{'EndNodes','Weight'});
    edge_delaunay = [];
elseif size(region_cm,1)==2
    EdgeTable = table([1 2],sum((region_cm(1,:)-region_cm(2,:)).^2),'VariableNames',{'EndNodes','Weight'});
    edge_delaunay = [idx_start idx_end];
else
...
elseif size(region_cm,1)==2
    via_points = region_cm([idx_start idx_end],:);
    path_radius = region_radius([idx_start idx_end]);
    Gsol.numedges = 1;
    Gsol.Edges.EndNodes = [idx_start idx_end];
else
